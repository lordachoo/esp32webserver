/*
 * Pages for ESP32-Async
 * These pages can be called from ESP32-Async and run through processor() function
 */

 const char pages_test[] PROGMEM = R"rawliteral(
<!DOCTYPE HTML>
<html lang="en">
<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.1.3/dist/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
    
  <title>ESP32 Web Server %ASYNCSERVERVER%</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="icon" href="data:,">
  <style>
    html {font-family: Arial; display: inline-block; text-align: center;}
    h2 {font-size: 3.0rem;}
    p {font-size: 3.0rem;}
    body {max-width: 600px; margin:0px auto; padding-bottom: 25px;}
  </style>
</head>
<body>
  <h2>ESP32 Web Server %ASYNCSERVERVER%</h2>
  pages.h compile test
%JAVASCRIPT0%
<script>
</script>
</body>
</html>
)rawliteral";

const char index_html[] PROGMEM = R"rawliteral(
<!DOCTYPE HTML>
<html lang="en">
<head>

    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.1.3/dist/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">

    
  <title>ESP32 Web Server %ASYNCSERVERVER%</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="icon" href="data:,">
  <style>
    html {font-family: Arial; display: inline-block; text-align: center;}
    h2 {font-size: 3.0rem;}
    p {font-size: 3.0rem;}
    body {max-width: 600px; margin:0px auto; padding-bottom: 25px;}
  </style>
</head>
<body>
  <h1>ESP32 Web Server</h1>
  ESP32 Web Server Version : %ASYNCSERVERVER%<BR>
  This Is running on an ESP32 [2x %CPUFREQ% MHz Xtensa 32-Bit LX6 Processor Cores] :<BR><BR>  
<center><img class="img-fluid" src='https://i.imgur.com/WpN1nNS.png'></center> <BR>
<h3>ESP32 Pages:</h3>
<ul>
<li><a href='/buttons'>GPIO Button Manipulation</a></li>
<li><a href='/scan'>Scan WiFi Networks (Local to ESP32 Webserver)</a></li>
<li><a href='/favicon.ico'>favicon.ico (Served from PROGMEM)</a></li>
</ul>
<BR><BR>
<div class="alert alert-primary" role="alert">
Page Loads: %PAGECOUNT% <BR>
Server Uptime: %UPTIME%
</div>
<pre>
TODO:
- HW Details/Config Page
   - SSD1306 OLED Pinout/HW Info / Library Info
   - MicroSD Card Holder Pinout / Library Info
   - Wiring Diagram 

- SD Card addition
   - Log to accessLog (as well as Serial)
   - Host the actual ESP32 C/Arduino code on SD via serveStatic (Link on front page) 
- Proper C defines for the OLED screen code being on or off
- Proper C defines for SD Card/SPIFF (for the includes and for any routes using SPIFF/serveStatic
- C Functions & Page for printing hash result of a get variable, with different hash varieties as the other get var
- Further templetize re-used HTML into the processor() function for speed/efficiency as re-usable variables

DONE:
- Init a github for the code, link it here -- Done [https://bitbucket.org/lordachoo/esp32-async-server/]
- Catch all route for 404 page?
- Add links/functions to the front page index_html -- added
- Bolt on bootstrap or something and make it look like slightly less shit -- done 11/3/22
- PAGE SERVED COUNT VAR/CHAR  -- Make it an LCD Update Loop (?) -- Added 11/3/22
- UPTIME variable in processor() -- done using uptime library (arduino)
</pre>

<!-- https://getbootstrap.com/docs/4.1/getting-started/introduction/ -->
%JAVASCRIPT0%
<script>
</script>
</body>
</html>
)rawliteral";


const char notFound_html[] PROGMEM = R"rawliteral(
<!DOCTYPE HTML>
<html lang="en">
<head>

    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.1.3/dist/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">

    
  <title>ESP32 Web Server %ASYNCSERVERVER% - 404</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="icon" href="data:,">
  <style>
    html {font-family: Arial; display: inline-block; text-align: center;}
    h2 {font-size: 3.0rem;}
    p {font-size: 3.0rem;}
    body {max-width: 600px; margin:0px auto; padding-bottom: 25px;}
  </style>
</head>
<body>
  <h1>ESP32 Web Server - 404</h1>
  404 - Page Not Found - <a href='/'>Back to Home</a>
<BR><BR>
<div class="alert alert-primary" role="alert">
Page Loads: %PAGECOUNT% <BR>
Server Uptime: %UPTIME%
</div>
<!-- https://getbootstrap.com/docs/4.1/getting-started/introduction/ -->
%JAVASCRIPT0%
<script>
</script>
</body>
</html>
)rawliteral";


const char buttons_html[] PROGMEM = R"rawliteral(
<!DOCTYPE HTML><html>
<head>
  <title>ESP32 Web Server %ASYNCSERVERVER%</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="icon" href="data:,">
  <style>
    html {font-family: Arial; display: inline-block; text-align: center;}
    h2 {font-size: 3.0rem;}
    p {font-size: 3.0rem;}
    body {max-width: 600px; margin:0px auto; padding-bottom: 25px;}
    .switch {position: relative; display: inline-block; width: 120px; height: 68px} 
    .switch input {display: none}
    .slider {position: absolute; top: 0; left: 0; right: 0; bottom: 0; background-color: #ccc; border-radius: 6px}
    .slider:before {position: absolute; content: ""; height: 52px; width: 52px; left: 8px; bottom: 8px; background-color: #fff; -webkit-transition: .4s; transition: .4s; border-radius: 3px}
    input:checked+.slider {background-color: #b30000}
    input:checked+.slider:before {-webkit-transform: translateX(52px); -ms-transform: translateX(52px); transform: translateX(52px)}
  </style>
</head>
<body>
  <h2>ESP32 Web Server %ASYNCSERVERVER%</h2>
  %BUTTONPLACEHOLDER%
<script>function toggleCheckbox(element) {
  var xhr = new XMLHttpRequest();
  if(element.checked){ xhr.open("GET", "/update?output="+element.id+"&state=1", true); }
  else { xhr.open("GET", "/update?output="+element.id+"&state=0", true); }
  xhr.send();
}
</script>
</body>
</html>
)rawliteral";

//File: favicon.ico.gz, Size: 726
#define favicon_ico_gz_len 726
const uint8_t favicon_ico_gz[] PROGMEM = {
 
};
